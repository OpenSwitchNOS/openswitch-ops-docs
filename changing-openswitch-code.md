...
### Workflows
There are basic scenarios in the development environment where you'll have to use the `devenv_` commands. This section gives you workflow examples that can help you get familiar with the commands.
...
* You can run the individual test case by specifying it on the command line:
```
...
$ make devenv_init
```
If git-review has been installed, `make`-ing the `devenv_init` target configures the workspace for possible future change submissions.
