...
* Step 6: Modify the service file
...
Process hardening is the act of reducing an executables attack surface.  This means reducing the privilege levels where possible and limiting how the process can access OS resources.  Reducing the privilege level for OPS means changing OPS daemons that currently run with root privileges, to instead run as a normal user, wherever possible.  The **opsd** user has been created for that purpose.  The opsd user's primary group is **ovsdb-client**.
...
Firejail is going to be the tool used to perform “Process Isolation” or “Process Hardening” (https://firejail.wordpress.com/):
...
Currently all OPS daemons run as root.  Wherever possible, we want to run the daemons under the **opsd** user. This should be the first choice for all daemons. If for some reason a daemon requires more permission/groups, contact the infra team (infra@lists.openswitch.net) to have the permissions/groups added.  For example, one or more daemons need to grant elevated permissions to only a specific group and this group does not currently exist.
...
- Run the tests again, if the process crashes a log will be set to syslog (for OPS, syslog is kept in /var/log/messages) or /var/log/audit/auditd.log, depending which is set. A log like the following will appear
...
**WARNING:** Because the OPS Docker images currently use the AUFS storage driver, Linux capabilities are not implemented and the setcap/getcap utilities will fail.  The BTRFS storage driver does have the ability to set capabilities on files. Using the BTRFS driver on OPS docker images is being investigated.  Linux capabilities do work correctly on OPS switches.



When a process runs as **root** it will by default have enabled all **"Linux Capabilities"**.  It is important to block all "Linux Capabilities" an ops-process is not going to use, to prevent unwanted access to a resource in the system. To see the full list of "Linux Capabilities" and their use take a look at:  http://man7.org/linux/man-pages/man7/capabilities.7.html



 **Note**: if no Linux Capabilities are needed, the process should be running under **opsd** user and be using seccomp filtering.
...
To test that the process is running with the correct minimal subset of capabilities we are going to use the **setcap** and **getcap** commands to modify and check the capabilities set on a process. For more information please look at: https://wiki.archlinux.org/index.php/Capabilities
...
- Server556 is a program that will open a socket and bind to port 556. To open any port under 1024 the **CAP_NET_BIND_SERVICE** capability is needed. If this process where to be run by **opsd** a "permission denied" error would occur.
...
Firejail can block certain protocols used to open sockets. The ones that it can block are **uinix, inet, inet6, netlink and packet**. It does it through seccomp checking the first argument made to the socket system call.
...
#Default System Calls blacklist, please see https://firejail.wordpress.com/features-3/man-firejail/ as reference
...
**Note**: For more information about profiles please look at: https://firejail.wordpress.com/features-3/man-firejail-profile/
...
In the **`<process-name>`.service** file modify the **ExecStart** to include Firejail.  An example:
		...
		ExecStart=/usr/bin/firejail --user=opsd --force -- /usr/bin/restd



The **-\-user** option will tell Firejail to run the process on behalf of the specified user (opsd in this case). If you which to run as **root** omit this option.
...
- If the process is a **python script** the capability must be set on the **python interpreter**.  Setting a capability on the python interpreter would cause all Python scripts to have that capability.  This is highly undesirable. It is recommended you create a copy of the python interpreter under a different name and set the capability on that.
...
- All the profile options can be added by command line (please take a look at https://firejail.wordpress.com/features-3/man-firejail/), making it easier in some occasions to add and remove options, e.g. **firejail -\-noprofile -\-private-bin=bash,ls,cat,sed -\- bash**
...
- Firejail has a monitoring program which allows to monitor the jails and their behaviours. **firemon** is the programs name, more information can be found at https://firejail.wordpress.com/features-3/man-firemon/
...
- If docker is using the storage driver **AUFS** Linux Capabilities can't be set on a file. The recommended storage driver is **BTRFS**.
...
- For cases where firejail has to be enabled for given user login. You'll need to use a wrapper user_init.sh which is added as the startup script for that user login. For example:
...
- Combining seccomp filtering and enabling Capabilities through **setcap** will not work correctly. We are currently looking at this issue.
...
This section contains the investigation results of what ir needed to setup a docker image to use the BTRFS storage driver.  While you might be able to change your local VM to use the BTRFS driver, none of the code changes that depend on this driver (e.g. linux capabilities) can be check in until all developer VMs and all Zuul test VMs are also changed to use the BTRFS driver.
...
The first step was to log in to a docker image and edit the restd service file to add a "User" and "Group" statements to set **opsd** as the user and **ovsdb-client**" as the group respectively.  Then a **systemctl restart restd** command was issued.  As expected the restd service failed to start.  Looking at the /var/log/messages file showed that restd received a "Permission denied" error when trying to create the /var/run/persistent_cookie_secret file.  Looking at the permissions for /var/run/ which is a link to the /run directory shows that it is only writable by root.  Reverse engineering the source code shows that the cookiesecret.py file in the ops-aaa-utils repo creates the file.
...
Here is a case of a file operation requiring root privileges that required a work around.  To work around this issue, the following changes needed to be made.  Modify the cookiesecret.py file to create the file in the /var/run/aaa/ directory instead of the /var/run/ directory.  Modify the aaautils.service file and add the following two lines:
	...
	ExecStartPre=-/bin/mkdir -m 0700 /var/run/aaa
...
Given this fact, all port changes were restored to use 443.  The work around is to use the Linux CAP_NET_BIND_SERVICE capability on the restd process to allow restd to bind to the privileged port.  However, it was discovered the Linux capabilities are not implemented for our docker containers when using AUFS storage driver.  Since the gate tests only run on the docker container, the conclusion is that restd cannot run as a non-root user.

This is can become an obstacle and be a show stopper, if it is must to be run as a non-root, some workarounds could be:
- Change the storage driver in ALL OPS docker containers.
- Modify the old test framework to allow the RestCmd() caller to override the HTTPS port to use.
- Enhance FIrejail to be able to set the capabilities before running the executable.