 ...
 * The contributor uses the Gerrit Review system to finally approve the change (log in to the Gerrit Review system and give the change a +1 rating).
 ...
 * The contributor uses the git commands (git add, commit) to include those changes in the local copy of the repository.
 * The contributor uses the review command with the amend switch (git review --amend) to include the new changes in the remote Gerrit Review system .
...
$ git status
```
* If the file listed in the output is ready to be submitted for review, run the following commands. It is the same command as the initial change but the review command has an extra switch (--amend) that indicates that this is part of a previous change.
```bash
$ git add <filename>
$ git commit -s -m "<a mandatory message with a description of the changes>"
$ git review --amend
