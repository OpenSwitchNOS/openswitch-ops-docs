...
After following the above install, configure and log in instructions, the newly created Ubuntu VM has been fully installed with the OpenSwith Build System and Development Environment. The OpenSwitch Build creates and maintains a Git repository that is cloned to `~/ops-build` in the newly created Ubuntu VM. The Git repository contains a subset of the project's source files. For details on how to manage it, see the [Development Environment](./development-environment.html) guide.
