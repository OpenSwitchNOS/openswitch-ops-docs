...
Defects in software are considered errors or bugs that create a discrepancies between actual and expected results of the system functionalities. 
...
Defects will follow the below flow in most cases. However, exceptions to this regular flow may occur, hence special handling may be required.

The detail on how to manage a defect in the OpenSwitch project will be explained on the following sections.

![enter image description here](https://lh3.googleusercontent.com/-DYjRh0qJUoM/VgRZgOR1kXI/AAAAAAAAAL8/zTaAb404-uQ/s0/defect-workflow.jpg "defect-workflow.jpg")
...
Submitted defects will be assigned to engineers in charge of start a review process. 
The defect information provided by submitters will be crucial in this phase because it will allow engineers get an understanding of the issue, do a root cause analysis and, if possible, implement a fix for the bug. 
...
**Assigned:** After the Component/Feature owner determines the defect contains everything that is needed to root cause it, will proceed to assign an engineer as Owner so that the root cause analysis begins.

**More Info:** In case the Component/Feature owner determines that there is not enough information has provided to proceed with defect investigation, the defect state will be set to "More Info" and send back to submitter. The reasons for this states could be that the engineer do not have the required information to reproduce the issue or relevant defect information is pending.  
...
**Skip-fix: ** The engineer has completed the investigation and is recommending not to fix the defect.  This status could be transitioned later on to either deferred or to a closed state.
